...
echo "cpuset:"$cpuset
	...
	CMD="taskset -c $cpuset snort -z 0  -v --rule-path /usr/local/etc/rules/ -c /usr/local/etc/snort/snort.lua --lua '$HYPERSCAN' $PCAP_LIST_OPT 2>&1 > ./results-numa/snort-$numa-$snum-cores-$search_mode-results-mthread-test.txt"
fi
